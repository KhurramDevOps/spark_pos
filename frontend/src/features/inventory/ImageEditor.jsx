  ...
  return (
    <div className="space-y-2">
      <div className="flex items-start gap-3">
        ...
        <div
          ...
          className={`flex-1 rounded border-2 border-dashed p-3 text-center text-xs transition ${
            dragOver ? "border-indigo-400 bg-indigo-50 dark:bg-indigo-950/40" : "border-line"
          }`}
        >
      ...
      <div className="flex items-center gap-3">
        {image && (
          <button type="button" onClick={remove} disabled={busy} className="text-xs text-red-600 dark:text-red-400 hover:underline disabled:opacity-50">
            Remove image
          </button>
        )}
        {busy && <span className="text-xs text-fg-subtle">Working…</span>}
        {notice && <span className="text-xs font-medium text-green-600 dark:text-green-400">{notice}</span>}
