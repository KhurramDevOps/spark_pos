...
function ExpensesList({ onEdit }) {
  ...
  if (isLoading) return <p className="text-sm text-fg-muted">Loading expenses…</p>;
  if (expenses.length === 0)
    return <p className="text-sm text-fg-muted">No expenses recorded yet.</p>;
      ...
      {expenses.map((exp) => (
        <div key={exp._id} className="flex items-center justify-between gap-3 px-4 py-2.5 text-sm">
          <div className="flex items-center gap-3">
            ...
            <span className="text-fg-muted">{exp.date?.slice(0, 10)}</span>
            {exp.note && <span className="text-fg-subtle">— {exp.note}</span>}
          </div>
          <div className="flex items-center gap-3">
            <span className="tabular-nums font-medium text-fg">
              {formatPaisa(exp.amount?.$numberDecimal ?? exp.amount)}
            </span>
            <button className="text-xs text-accent hover:underline" onClick={() => onEdit(exp)}>
              Edit
            </button>
            <button
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-6">
      <header className="mb-5 flex items-start justify-between">
        <div>
          <h1 className="text-xl font-semibold text-fg">Daily close</h1>
          <p className="text-sm text-fg-muted">Did the drawer balance, and was the day worth it?</p>
        </div>
      ...
      <section className="mt-8">
        <h2 className="mb-2 text-sm font-semibold text-fg">Expenses</h2>
